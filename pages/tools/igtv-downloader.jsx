...
const IGTVDownload = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(instaPostId(link?.trim()));
    if (validURL(link)) {
      setShortcode(instaPostId(link.trim()));
      const code = instaPostId(link.trim());
      if (code) {
        setIsLoading(true);
        fetchPost(code, setIsLoading, setShowVideo, setPosts, savePostToDB,theme);
      } else {

...
const PostCard = ({ userData, post, reels, shortcode }) => {
  console.log({ thumbnail_src: post?.main_media_hd });
  return (
    <div className="w-full p-2 border dark:border-none bg-gray-100 dark:bg-dark-400 shadow-lg rounded-lg">
      <div className="relative">
        ...
        {post?.main_media_type === "video" && (
          <a href={post?.main_media_hd} target="_blank" rel="noreferrer">
            <PlayIcon className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-7xl opacity-70" />
          </a>
        )}

        <a href={post?.main_media_hd} target="_blank" rel="noopener noreferrer">
          {post?.main_media_type === "video" ? (
            <div
            ...
            <img
              src={post?.main_media_hd || "https://i.stack.imgur.com/6M513.png"}
      ...
      <div className="mt-2 border dark:border-none p-2">
        <div className="grid grid-cols-2 gap-2">
          {post?.child_medias_hd?.map((item, index) => (
            <a
              href={item?.url}
              target="_blank"
              rel="noreferrer"
